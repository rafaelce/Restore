...
ApplicationContext context, IConfiguration config, ILogger<PaymentsController> logger) : BaseApiController
{
    [Authorize]
    [HttpPost]
    ...
    private Event ConstructStripeEvent(string json)
    {
        try
        {
            return EventUtility.ConstructEvent(json,
                Request.Headers["Stripe-Signature"], config["Stripe:WhSecret"]);
        }
        catch (Exception ex)
        {
